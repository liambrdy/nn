...
void plot_cost_minmax(Cost_Plot plot, float *min, float *max)
{
    *min = FLT_MAX;
    *max = FLT_MIN;
...
int main(int argc, char **argv)
{
    ...
    while (!WindowShouldClose()) {
        ...
        ClearBackground(background_color);
        {
            ...
            char buffer[256];
            snprintf(buffer, 256, "Epoch: %zu/%zu, Rate: %f, Cost: %f", epoch, max_epoch, rate, nn_cost(nn, ti, to));
            DrawText(buffer, 0, 0, h*0.04f, WHITE);
        }
        EndDrawing();
    }
